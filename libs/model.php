...
///////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function gp_qry_all_website_by_user($v_lang_id)
    {
        ...
        $stmt            = "Select a.PK_WEBSITE,a.C_NAME from ($stmt) a ";
                ...
                else if(DATABASE_TYPE == 'MYSQL')
                {
                    ...
                    $sql       = "
                        Select
                              ...
                              , A.C_SUMMARY
                              , A.C_BEGIN_DATE
                              , DATE_FORMAT(A.C_BEGIN_DATE, '%d-%m-%Y') AS C_BEGIN_DATE_DDMMYYY
                              , A.C_HAS_VIDEO
                              , A.C_HAS_PHOTO
...
/*
    ...
    */
    ...
    public function gp_qry_all_menu_position($website_id = 0,$menu_position_id = NULL)
    {
        $website_id = replace_bad_char($website_id);
        if ($website_id == 0)
        {
        ...
        if($menu_position_id >0)
        {
            $row['@attributes'] = array('position_menu_id' => $menu_position_id,
                                        'position_code' =>'widget_menu');
            $r[] = $row;
        }
        else
        {
        ...
        for($count=0;$count<count($r);$count++)
        {
            ...
            {       
                ...
                elseif ($v_menu_type == 'module')
                {
                    ...
                    //Cau hoi khao sat
                    else if($v_value_module == 'dsp_survey')
                    {
                        $DATA_MODEL[$v_position_code][$i]['C_URL'] = build_url_survey();
                    }
                    //Cau hoi khao sat
                    else if($v_value_module == 'faq')
                    {
                        $DATA_MODEL[$v_position_code][$i]['C_URL'] = build_url_cq($website_id);
                    }
                    //Cau hoi khao sat
                    else if($v_value_module == 'help')
                    {
                        $DATA_MODEL[$v_position_code][$i]['C_URL'] = build_url_page_help($website_id);
                    }
                    //Gui thong tin lien he
                    else if($v_value_module == 'contact')
                    {
                        $DATA_MODEL[$v_position_code][$i]['C_URL'] = build_url_contact($website_id);
                    }
                    //thong tin toa soan
