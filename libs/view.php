...
//        $this->template->local_js       = SITE_ROOT . 'apps/' . $app . '/modules/' . $module . '/' . $module . '_views/js_' . $module . '.js';
        $this->local_js       = SITE_ROOT . 'apps/' . $app . '/modules/' . $module . '/' . $module . '_views/js_' . $module . '.js';
    ...
    public function render_form_display_all($data, $edit = TRUE)
    {
        ...
        $html .= '<table class="table table-striped table-bordered table-hover" width="100%" class="adminlist" cellspacing="0" border="1" style="margin-bottom:0px">' . $table_col_size
                . '<tr>' . $table_header . '</tr></table>';

        //List item
        $html .= '<table class="table table-striped table-bordered table-hover" width="100%" class="adminlist" cellspacing="0" border="1">';
        $html .= $table_col_size;
        $i = 0;
        for ($i = 0; $i < sizeof($data); $i++)
        {
            $v_row_class = 'row' . ($i % 2);
            $html .= '<tr height="20" role="presentation" data-item_id="' . $data[$i][$primarykey] . '">';
    ...
    public static function add_empty_rows($pCurrentRow, $pTotalRow, $pTotalColumn)
    {
        ...
        for ($i = $pCurrentRow + 1; $i <= ($pTotalRow ); $i++)
        {
            $v_row_class = 'row' . ($i % 2);
    ...
    public static function generate_select_option($arrData, $selected, $public_xml_file_name = '')
    {
        ...
        {   
            foreach ($arrData as $key => $val)
            {
                ...
                //$str_selected = ($key == strval($selected)) ? ' active' : '';
                //$html .='<li tabindex="0" aria-controls="dataTables-example" class="paginate_button'. $str_selected .'"><a href="#">' . $val . '</a></li>';
