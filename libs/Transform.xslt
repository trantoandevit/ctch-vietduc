...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
							...
							<xsl:if test="@label!=''">
								<table class="table table-striped table-hover" border="0">
									<tr class="panel_color">
										<td colspan="4" height="25px" style="background-color:lightgrey;">
											<span class="@css">
  ...
  Call this template when object is normal text box-->
	<xsl:template name="CreateTextboxArea">
		<xsl:if test="@id='txtTaiLieuKhac'">
			<textarea name="{@id}" id="{@id}" cols="{@size}" class="form-control valid" style="display:none;" rows="10" value="{@defaul_value}" data-allownull="{@allownull}" data-validate="{@validate}" data-name="{@name}" data-xml="yes" data-doc="{@doc}">
				<xsl:value-of select="@defaul_value"/>
			</textarea>
		</xsl:if>
		<xsl:if test="@id!='txtTaiLieuKhac'">
			<textarea name="{@id}" id="{@id}" cols="{@size}" class="form-control  valid" rows="10" value="{@defaul_value}" data-allownull="{@allownull}" data-validate="{@validate}" data-name="{@name}" data-xml="yes" data-doc="{@doc}" data-editor="{@editor}">&#x20;
				<xsl:value-of select="@defaul_value"/>
			</textarea>
		</xsl:if>
	</xsl:template>
  ...
  Call this template when object is normal text box has even-->
	<xsl:template name="CreateTextboxMoney">
		<input type="textbox" id="{@id}" class="form-control  valid" size="{@size}" value="{@defaul_value}" onfocusout="ReadNumberToString('{@id}','lbl_mess_{@id}');" onkeyup="{@Even}" maxlength="15" onKeyDown="return handleEnter(this, event);"
  ...
  Call this template when object is normal text box has even-->
	<xsl:template name="CreateTextboxName">
		...
		<input type="textbox" id="{@id}" class="form-control  valid" size="{@size}" value="{@defaul_value}" onkeyup="{@Even}" onKeyDown="return handleEnter(this, event);" data-allownull="{@allownull}" data-validate="{@validate}" data-name="{@name}" data-xml="yes"
		       data-doc="{@doc}"/>
		<br/>
		<span id="lbl_mess_{@id}" class="{@css}"></span>
		<!--b?t các ki?u d? li?u c?a textbox-->
  ...
  Call this template when object is password-->
	<xsl:template name="CreateTextPassword">
		<input type="password" id="{@id}" class="form-control  valid" size="{@size}" value="{@defaul_value}" onkeyup="{@Even}" onKeyDown="return handleEnter(this, event);" data-allownull="{@allownull}" data-validate="{@validate}" data-name="{@name}" data-xml="yes"
		       data-doc="{@doc}"/>
		<xsl:if test="@legend != ''">
			<label>
				<xsl:value-of select="@legend" disable-output-escaping="yes"/>
