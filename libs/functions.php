...
function get_post_var($html_object_name, $default_value = '', $is_replace_bad_char = TRUE)
{
    ...
    if (!is_array($is_replace_bad_char) && $is_replace_bad_char)
    {
...
function get_get_var($html_object_name, $default_value = '', $is_replace_bad_char = TRUE)
{
    $var = isset($_GET[$html_object_name]) ? $_GET[$html_object_name] : $default_value;
...
#Lien he
function build_url_contact( $website_id)
{
    if (check_file_htaccess())
    {
        $url = SITE_ROOT . "lien-he" . '/' . $website_id;
    }
    else
    {
        $url = SITE_ROOT . "index.php?url=frontend/frontend/dsp_contact/&website_id=$website_id";
    }
    return $url;
}
 ...
 * @return type
 */
function build_url_single_record_type($record_type_id,$website_id)
{
    if($record_type_id >0)
    {
        $url = SITE_ROOT;
        if(check_file_htaccess())
        {
           $url .= "dich_vu_cong/danh_sach/chi-tiet/$record_type_id-$website_id";
        }
        else
        {
            $url .= "index.php?url=frontend/frontend/dsp_single_record_type?record_type_id=$record_type_id&website_id=$website_id";
        }
        return $url;
    }
}
...
function build_url_cq_detail($website_id, $v_slug, $cq_id)
{
    if (check_file_htaccess())
    {
        $url = SITE_ROOT . "hoi-dap/tra-loi/$v_slug/$website_id-$cq_id";
    }
    else
    {
...
function replace_video($m)
{
    ...
    $v_style = '';
    $count_dr = count(explode('/', $m[1]));      
    if ($count_dr == 1)
    {
        //is Youtube.
        $youtube_url = 'https://www.youtube.com/embed/' . $m[1];
        $html = '<div style="width:100%;height:400px;margin:0 auto"><iframe width="100%" height="100%" src="' . $youtube_url . '" frameborder="0" allowfullscreen></iframe>'
                . ' </div>';
    }
    else
    {
        ...
        elseif($v_file_extension == 'swf')
        {
            $v_provider = 'swf';
            $html = ' <div style="width:100%;float:left">
                    <center>
                            <div class="video_container" ' . $v_style . '>
                                <embed  id="ply' . $count . '"  src="' .  $m[1] . '" width="' . $v_width . '" height="' . $v_height . '"  />
                            </div>  
                    </center>
                </div>';
        }
...
function build_url_citizen_reset_password()
{
    
    $url = FULL_SITE_ROOT;
    if(check_file_htaccess())
    {
        $url .= "tai-khoan/tim-mat-khau";
    }
    else
    {
        $url = FULL_SITE_ROOT . "index.php?url=frontend/frontend/dsp_form_request_password";
    }
    return $url;
}
...
function call_soap_service($client,$function,$arr_param = array(),$recursive = false,$index=0)
{
 ...
 function build_url_page_help($website_id,$v_id = 0)
{
    $url = SITE_ROOT;
    if(check_file_htaccess())
    {
        $url .=  "dich_vu_cong/huong-dan/$website_id-$v_id";
    }
    else
    {
        $url = SITE_ROOT . "index.php?url=frontend/frontend/dsp_help?website_id=$website_id&id=$v_id";
    }
    return $url;
}