...
//        Session::check_permission('QL_DANH_SACH_MEDIA', false) or $this->access_denied();
    ...
    function thuvien()
    {
        ...
        Session::set(EXT_SESSION_KEY, str_replace(' ', '', $this->extenstions));
        $this->view->set_layout('pop_win_layout')->render('dsp_main', $data);
    }
...
//        if (file_exists($fname) && Session::check_permission('XEM_DANH_SACH_MEDIA', false))
        {
            $data['fname'] = $fname;
            $data['furl']  = $furl;
            $this->view->set_layout(NULL)->render('dsp_item_details', $data);
        }
    }
