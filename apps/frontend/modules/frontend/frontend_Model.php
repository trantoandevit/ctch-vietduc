...
class frontend_Model extends Model
{
    ...
    {
        parent::__construct();
        $this->db->Execute("SET SESSION group_concat_max_len = 999999999;");
    }
     ...
     * @return type
     */
    public function qry_all_member_receive_internet($record_type_id = 0)
    {
        $condition = '';
        if ($record_type_id > 0)
        {
        ...
        $sql = "SELECT DISTINCT M.PK_MEMBER,
                                M.C_NAME,
                                CONCAT(tm.C_CODE_MAPPING,'-',M.C_SHORT_CODE) AS C_CODE
                        FROM t_ps_member M
                          INNER JOIN t_ps_record_type_member RTM
                            ON M.PK_MEMBER = RTM.FK_MEMBER
                             LEFT JOIN (SELECT *
                                       FROM t_ps_record_type_member
                                       WHERE 1=1 $condition) tm
                              ON tm.FK_MEMBER = M.PK_MEMBER
                        WHERE (M.FK_MEMBER < 1
                                OR M.FK_MEMBER IS NULL)
                                and  tm.C_CODE_MAPPING != ''
                            AND M.C_STATUS = 1
                            group by M.PK_MEMBER";
     ...
     * @return type
     */
    public function qry_all_record_receive_internet($sel_fields_id = '', $v_member_id = '', $v_text_filter = '')
    {
        ...
        if ($sel_fields_id > 0)
        {
            $condition_spec .= "   AND RT.C_SPEC_CODE = (SELECT C_CODE FROM t_cores_list WHERE PK_LIST = '$sel_fields_id' AND C_STATUS = 1) ";
        }
        if ($v_text_filter != '')
        {
            $condition_filter .= " AND (RT.C_CODE LIKE '%$v_text_filter%' OR RT.C_NAME like '%$v_text_filter%')";
        }
        $stmt = "SELECT
                  ...
                  FROM  (SELECT @rownum := 0) r
                            ,(SELECT DISTINCT FK_RECORD_TYPE FROM t_ps_record_type_member WHERE (1 > 0) 
                            $condition_member
                        ) RTM
                    LEFT JOIN t_ps_record_type RT
                      ON RTM.FK_RECORD_TYPE = RT.PK_RECORD_type
                  WHERE (1 > 0) $condition_spec $condition_filter
                  ORDER BY C_SPEC_CODE";
        $stmt = "SELECT
                    @rownum:=@rownum +1 AS RN,
                    (SELECT
                            count(RT.PK_RECORD_TYPE)
                            FROM  (SELECT @rownum := 0) r
                            ,(SELECT DISTINCT FK_RECORD_TYPE FROM t_ps_record_type_member WHERE (1 > 0) 
                            $condition_member
                        ) RTM
                    LEFT JOIN t_ps_record_type RT
                      ON RTM.FK_RECORD_TYPE = RT.PK_RECORD_type
                  WHERE (1 > 0) $condition_spec $condition_filter
                  ORDER BY C_SPEC_CODE)  As C_TOTAL_ROW,
                    RT.PK_RECORD_TYPE,
                    RT.C_CODE,
                    RT.C_NAME,
                    RT.C_SPEC_CODE,
                  ...
                  FROM  (SELECT @rownum := 0) r
                    ...
                    LEFT JOIN t_ps_record_type RT
                      ON RTM.FK_RECORD_TYPE = RT.PK_RECORD_type
                  WHERE (1 > 0) $condition_spec $condition_filter
                  ORDER BY RN Limit $v_start,$limit";
    ...
    public function qry_all_spec()
    {
        $sql = "SELECT C_CODE,C_NAME
                ...
                WHERE FK_LISTTYPE = (SELECT
                                       PK_LISTTYPE
                                     FROM t_cores_listtype
                                     WHERE C_CODE = '" . _CONST_LINH_VUC_TTHC . "') $condition";
        return $this->db->GetAssoc($sql);
    }

    /**
     * lay danh muc linh vuc co thu tuc level 3
     * @return array()
     */
    public function qry_all_fileds_level_3()
    {
        $sql = "SELECT
                        PK_LIST,
                        C_NAME
                      FROM t_cores_list l
                        RIGHT JOIN (SELECT
                                     DISTINCT C_SPEC_CODE
                                    FROM t_ps_record_type rt
                                      RIGHT JOIN (SELECT DISTINCT
                                                    DISTINCT FK_RECORD_TYPE
                                                  FROM t_ps_record_type_member rt
                                                    RIGHT JOIN t_ps_member m
                                                      ON m.PK_MEMBER = rt.FK_MEMBER
                                                  WHERE m.C_STATUS = 1) rtm
    ...
    public function do_insert_internet_record()
    {
        ...
        $response['field'] = array('hdn_member_id'        => $v_member_id, 'hdn_record_type_id'   => $v_record_type_id,
            'hdn_record_type_code' => $v_record_type_code, 'txt_record_no'        => $v_record_no,
        ...
        for ($i = 0; $i < $v_count_file; $i++)
        {
            ...
            if ($_FILES['uploader']['error'][$i] == 0)
            {
                ...
                if (in_array($v_file_ext, explode(',', trim(_CONST_TYPE_FILE_ACCEPT))))
                {
                    ...
                    {
                        $response['message']['uploader'] = 'Dung lượng tập tin đính kèm không được vượt quá ' . _CONST_LIMIT_FILE_SIZE . 'Mb';
                        break;
                    }
                }
                else
        ...
        if ($this->db->Affected_Rows() > 0)
        {
            ...
            for ($i = 0; $i < $v_count_file; $i++)
            {
                if ($_FILES['uploader']['error'][$i] == 0)
                {
                    ...
                    {
                        $v_new_file_name = $v_record_no . '_' . uniqid() . '.' . $v_file_ext;
    ...
    public function qry_single_article($website_id, $category_id, $article_id)
    {
        ...
        $stmt = "SELECT
                        ...
                        (SELECT
                           C_IS_RADIO
                         FROM t_ps_category
                         WHERE PK_CATEGORY = ?) AS C_IS_RADIO,
                        (SELECT
                           C_TYPE
                         FROM t_ps_category
                         WHERE PK_CATEGORY = ?) AS C_TYPE,
                        (SELECT
                           C_SLUG
                         FROM t_ps_category
                      ...
                      WHERE PK_ARTICLE = ?
                          ...
                          AND C_BEGIN_DATE <= NOW()
                          AND C_END_DATE >= NOW()";
        $article = $this->db->getRow($stmt, array($category_id,$category_id,$category_id, $category_id, $article_id,$category_id, $website_id, $category_id));
        ...
        if ($check == $website_id)
        {
            if (DATABASE_TYPE == 'MSSQL')
            {
                $stmt = "INSERT INTO t_ps_cq (FK_FIELD,
                    ...
                    C_SLUG)
                    VALUES (?,?,?,?,?,?,?,?,?,GETDATE(),?)";
                $param = array($v_field_id, $v_address, $v_name, $v_phone, $v_email, $v_title, $v_content, '0', $v_order, $v_slug);
            }
            else if (DATABASE_TYPE == 'MYSQL')
            {
                $stmt = "INSERT INTO t_ps_cq (FK_FIELD,
...
//end func qry_all_latest_article_by_website
    ...
    public function get_lookup_link()
    {
        $sql = "SELECT
                ...
                ExtractValue(C_XML_DATA,'//item[@id=\"txt_lookup_type\"]//value') AS C_LOOKUP_TYPE_LINK
              FROM t_ps_member";
        return $this->db->GetAssoc($sql);
    }
     ...
     * @return type
     */
    public function qry_synthesis_of_village($month = '', $year = '', $member_code = '', $spec_code = '')
    {
        $condition_member = '';
        $condition_spec = '';
        if ($member_code != '')
        {
            $condition_member = " AND M.C_CODE = '$member_code'";
        }

        if ($spec_code != '')
        {
            $condition_spec = " AND hs.C_SPEC_CODE = '$spec_code'";
        }
        $prev_month = Date('m', mktime(0, 0, 0, $month - 1, date("d"), date("Y")));
        $prev_year = Date('Y', mktime(0, 0, 0, $month - 1, date("d"), $year));
        $sql = "SELECT
                    M.C_NAME,
                    M.C_CODE,
                    HS.C_UNIT_CODE,
                    HS.FK_VILLAGE_ID,
                    IF(KT.C_COUNT_KY_TRUOC>=0,KT.C_COUNT_KY_TRUOC,0) AS C_COUNT_KY_TRUOC,
                    ...
                    COALESCE(HS.C_COUNT_DA_TRA_KET_QUA,0) AS C_COUNT_DA_TRA_KET_QUA,
                    ...
                    COALESCE(HS.C_COUNT_LUY_KE,0) AS C_COUNT_LUY_KE
                  FROM (SELECT
                          FK_VILLAGE_ID,
                          C_NAME,
                          C_CODE,
                          (SELECT
                             C_CODE
                           FROM t_ps_member
                           WHERE PK_MEMBER = m.FK_MEMBER) AS C_PARENT_CODE
                        FROM t_ps_member m
                        WHERE COALESCE(m.FK_VILLAGE_ID,0) > 0
                            AND m.C_STATUS = 1) M
                    LEFT JOIN (SELECT
                                 hs.C_UNIT_CODE,
                                 hs.FK_VILLAGE_ID,
                                 SUM(C_COUNT_TONG_TIEP_NHAN) AS C_COUNT_TONG_TIEP_NHAN,
                                 SUM(C_COUNT_TONG_TIEP_NHAN_TRONG_THANG) AS C_COUNT_TONG_TIEP_NHAN_TRONG_THANG,
                                 SUM(C_COUNT_DANG_THU_LY) AS C_COUNT_DANG_THU_LY,
                                 SUM(C_COUNT_DANG_CHO_TRA_KET_QUA) AS C_COUNT_DANG_CHO_TRA_KET_QUA,
                                 SUM(C_COUNT_DA_TRA_KET_QUA) AS C_COUNT_DA_TRA_KET_QUA,
                                 SUM(C_COUNT_DANG_THU_LY_DUNG_TIEN_DO) AS C_COUNT_DANG_THU_LY_DUNG_TIEN_DO,
                                 SUM(C_COUNT_DANG_THU_LY_CHAM_TIEN_DO) AS C_COUNT_DANG_THU_LY_CHAM_TIEN_DO,
                                 SUM(C_COUNT_DA_TRA_KET_QUA_TRUOC_HAN) AS C_COUNT_DA_TRA_KET_QUA_TRUOC_HAN,
                                 SUM(C_COUNT_DA_TRA_KET_QUA_DUNG_HAN) AS C_COUNT_DA_TRA_KET_QUA_DUNG_HAN,
                                 SUM(C_COUNT_DA_TRA_KET_QUA_QUA_HAN) AS C_COUNT_DA_TRA_KET_QUA_QUA_HAN,
                                 ...
                                 SUM(C_COUNT_LUY_KE) AS C_COUNT_LUY_KE
                               FROM (SELECT
                                       C_UNIT_CODE,
                                       MAX(C_HISTORY_DATE) AS C_MAX_HISTORY_DATE,
                                       FK_VILLAGE_ID
                                     FROM t_ps_record_history_stat
                                     WHERE MONTH(C_HISTORY_DATE) = '$month'
                                         AND YEAR(C_HISTORY_DATE) = '$year'
                                         AND COALESCE(FK_VILLAGE_ID,0) > 0
                                     GROUP BY C_UNIT_CODE,FK_VILLAGE_ID) a
                                 LEFT JOIN t_ps_record_history_stat hs
                                   ON a.C_UNIT_CODE = hs.C_UNIT_CODE
                                     AND a.C_MAX_HISTORY_DATE = hs.C_HISTORY_DATE
                                     AND a.FK_VILLAGE_ID = hs.FK_VILLAGE_ID 
                                     Where (1>0) $condition_spec
                               GROUP BY hs.C_UNIT_CODE,FK_VILLAGE_ID) HS
                      ON M.C_PARENT_CODE = HS.C_UNIT_CODE
                        AND M.FK_VILLAGE_ID = HS.FK_VILLAGE_ID
                    LEFT JOIN (SELECT
                                hs.C_UNIT_CODE,
                                hs.FK_VILLAGE_ID,
                                SUM(C_COUNT_DANG_THU_LY) AS C_COUNT_KY_TRUOC
                              FROM (SELECT
                                      C_UNIT_CODE,
                                      MAX(C_HISTORY_DATE) AS C_MAX_HISTORY_DATE,
                                      FK_VILLAGE_ID
                                    FROM t_ps_record_history_stat
                                    WHERE MONTH(C_HISTORY_DATE) = '$prev_month'
                                        AND YEAR(C_HISTORY_DATE) = '$prev_year'
                                        AND COALESCE(FK_VILLAGE_ID,0) > 0
                                    GROUP BY C_UNIT_CODE,FK_VILLAGE_ID) a
                                LEFT JOIN t_ps_record_history_stat hs
                                  ON a.C_UNIT_CODE = hs.C_UNIT_CODE
                                    ...
                                    AND a.FK_VILLAGE_ID = hs.FK_VILLAGE_ID
                                    Where (1>0) $condition_spec
                              GROUP BY hs.C_UNIT_CODE,FK_VILLAGE_ID) KT
                     ON M.C_PARENT_CODE = KT.C_UNIT_CODE
                        AND M.FK_VILLAGE_ID = KT.FK_VILLAGE_ID
                    WHERE (1>0) $condition_member
                  ORDER BY M.C_PARENT_CODE";
        return $this->db->getAll($sql);
    }
     ...
     * @param type $spec_code
     */
    public function qry_synthesis($month = '', $year = '', $member_id = '', $spec_code = '')
    {
        $arr_return = array(); //arr value tra ket qua
        //lay thong tin don vi thanh vien
        $arr_member_info = array();
        if ($member_id != '')
        {
            $arr_member_info = $this->db->GetRow("SELECT C_CODE,C_SCOPE,FK_VILLAGE_ID FROM t_ps_member WHERE PK_MEMBER = '$member_id'");
        }
        $v_member_scope = empty($arr_member_info['C_SCOPE']) ? 0 : $arr_member_info['C_SCOPE'];
        $v_member_code = empty($arr_member_info['C_CODE']) ? '' : $arr_member_info['C_CODE'];

        //lay thong tin tong hop cac huyen
        $arr_district = $this->qry_synthesis_of_district($month, $year, $v_member_code, $spec_code);

        //lay thong tin tong hop cac xa
        $arr_village = $this->qry_synthesis_of_village($month, $year, ($v_member_scope == 2) ? $v_member_code : '', $spec_code);


        if ($v_member_scope != 2)//neu don vi lua chon la cap so hoac huyen
        {
            foreach ($arr_district as $district)
            {
                $v_unit_code = $district['C_UNIT_CODE'];
                array_push($arr_return, $district);
                foreach ($arr_village as $key => $village)
                {
                    $v_parent_unit_code = $village['C_UNIT_CODE'];
                    if ($v_parent_unit_code != $v_unit_code)
                    {
                        continue;
                    }
                    $village['C_NAME'] = ' -- ' . $village['C_NAME'];
                    array_push($arr_return, $village);
                    unset($arr_village[$key]);
                }
            }
        }
        else//neu don vi lua chon la cap xa
        {
            $arr_return = $arr_village;
        }
        return $arr_return;
    }

    /**
     * lay so lieu tong hop cua cac don vi cap huyen
     ...
     * @return type
     */
    public function qry_synthesis_of_district($month = '', $year = '', $unit_code = '', $spec_code = '', $has_village = 0)
    {
        $condition_spec = '';
        $condition_unit = '';
        $condition_has_village = '';
        $condition_has_village_group = '';
        ...
        if ($unit_code != '')//dieu kien ma don vi
        {
            $condition_unit = " AND M.C_CODE = '$unit_code'";
        }

        if ($spec_code != '')//dieu kien linh vuc
        {
            $condition_spec = " AND hs.C_SPEC_CODE = '$spec_code'";
        }

        if ($has_village == 0)//ko tinh xa
        {
            $condition_has_village = " AND COALESCE(hs.FK_VILLAGE_ID,0)=0";
            $condition_has_village_group = " AND a.FK_VILLAGE_ID = hs.FK_VILLAGE_ID";
        }
        else//co tinh ca xa
        {
            //chi lay nhung xa duoc cau hinh trong member
            $condition_has_village = " AND hs.FK_VILLAGE_ID IN (SELECT
                                                                    COALESCE(FK_VILLAGE_ID,0) AS FK_VILLAGE_ID
                                                                  FROM t_ps_member
                                                                  WHERE C_STATUS = 1
                                                                      AND (C_CODE = hs.C_UNIT_CODE
                                                                        OR FK_MEMBER = (SELECT
                                                                                  PK_MEMBER
                                                                                FROM t_ps_member
                                                                                WHERE C_CODE = hs.C_UNIT_CODE)))";
        }

        $prev_month = Date('m', mktime(0, 0, 0, $month - 1, date("d"), date("Y")));
        $prev_year = Date('Y', mktime(0, 0, 0, $month - 1, date("d"), $year));

        $sql_field = "  SUM(C_COUNT_TONG_TIEP_NHAN) AS C_COUNT_TONG_TIEP_NHAN,
                        SUM(C_COUNT_TONG_TIEP_NHAN_TRONG_THANG) AS C_COUNT_TONG_TIEP_NHAN_TRONG_THANG,
                        SUM(C_COUNT_DANG_THU_LY) AS C_COUNT_DANG_THU_LY,
                        SUM(C_COUNT_DANG_CHO_TRA_KET_QUA) AS C_COUNT_DANG_CHO_TRA_KET_QUA,
                        SUM(C_COUNT_DA_TRA_KET_QUA) AS C_COUNT_DA_TRA_KET_QUA,
        ...
        $stmt = "SELECT
                    ...
                    M.C_NAME,
                    IF(KT.C_COUNT_KY_TRUOC>=0,KT.C_COUNT_KY_TRUOC,0) AS C_COUNT_KY_TRUOC,
                    COALESCE(C_COUNT_TONG_TIEP_NHAN,0) AS C_COUNT_TONG_TIEP_NHAN,
                    COALESCE(C_COUNT_TONG_TIEP_NHAN_TRONG_THANG,0) AS C_COUNT_TONG_TIEP_NHAN_TRONG_THANG,
                    COALESCE(C_COUNT_DANG_THU_LY,0) AS C_COUNT_DANG_THU_LY,
                    COALESCE(C_COUNT_DANG_CHO_TRA_KET_QUA,0) AS C_COUNT_DANG_CHO_TRA_KET_QUA,
                    COALESCE(C_COUNT_DA_TRA_KET_QUA,0) AS C_COUNT_DA_TRA_KET_QUA,
                    COALESCE(C_COUNT_DANG_THU_LY_DUNG_TIEN_DO,0) AS C_COUNT_DANG_THU_LY_DUNG_TIEN_DO,
                    COALESCE(C_COUNT_DANG_THU_LY_CHAM_TIEN_DO,0) AS C_COUNT_DANG_THU_LY_CHAM_TIEN_DO,
                    COALESCE(C_COUNT_DA_TRA_KET_QUA_TRUOC_HAN,0) AS C_COUNT_DA_TRA_KET_QUA_TRUOC_HAN,
                    COALESCE(C_COUNT_DA_TRA_KET_QUA_DUNG_HAN,0) AS C_COUNT_DA_TRA_KET_QUA_DUNG_HAN,
                    COALESCE(C_COUNT_DA_TRA_KET_QUA_QUA_HAN,0) AS C_COUNT_DA_TRA_KET_QUA_QUA_HAN,
                    COALESCE(C_COUNT_CONG_DAN_RUT,0) AS C_COUNT_CONG_DAN_RUT,
                    COALESCE(C_COUNT_TU_CHOI,0) AS C_COUNT_TU_CHOI,
                    COALESCE(C_COUNT_BO_SUNG,0) AS C_COUNT_BO_SUNG,
                    COALESCE(C_COUNT_THU_LY_QUA_HAN,0) AS C_COUNT_THU_LY_QUA_HAN,
                    COALESCE(C_COUNT_THUE,0) AS C_COUNT_THUE,
                    COALESCE(C_COUNT_LUY_KE,0) AS C_COUNT_LUY_KE
                  FROM t_ps_member M
                    LEFT JOIN (SELECT
                                 hs.C_UNIT_CODE,
                                 hs.C_SPEC_CODE,
                               ...
                               FROM (SELECT
                                 ...
                                 LEFT JOIN t_ps_record_history_stat hs
                                   ON a.C_UNIT_CODE = hs.C_UNIT_CODE
                                     AND a.C_MAX_HISTORY_DATE = hs.C_HISTORY_DATE
                                     $condition_has_village_group
                                 WHERE (1>0) $condition_has_village $condition_spec 
                               GROUP BY hs.C_UNIT_CODE $sql_group) HS
                    ...
                    LEFT JOIN (SELECT
                              ...
                              FROM (SELECT
                                    ...
                                    WHERE MONTH(C_HISTORY_DATE) = $prev_month
                                        AND YEAR(C_HISTORY_DATE) = $prev_year
                                    GROUP BY C_UNIT_CODE) a
                                LEFT JOIN t_ps_record_history_stat hs
                                  ON a.C_UNIT_CODE = hs.C_UNIT_CODE
                                    AND a.C_MAX_HISTORY_DATE = hs.C_HISTORY_DATE
                                    $condition_has_village_group
                                WHERE (1>0) $condition_has_village $condition_spec 
                              GROUP BY hs.C_UNIT_CODE) KT
                     ON M.C_CODE = KT.C_UNIT_CODE
                  WHERE M.C_STATUS = 1 AND (M.C_SCOPE = 0 OR M.C_SCOPE = 1) $condition_unit ORDER BY M.C_NAME";

        return $this->db->getAll($stmt);
    }

    public function qry_synthesis_by_spec($month = '', $year = '', $member_id = '', $spec_code = '', $order_unit = 0)
    {
        $arr_return = array();
        //lay thong tin don vi thanh vien
        $arr_member_info = array();
        ...
        $arr_district = $this->qry_synthesis_by_spec_of_district($month, $year, $v_member_code, $spec_code);
        $arr_village = $this->qry_synthesis_by_spec_of_village($month, $year, ($v_member_scope == 2) ? $v_member_code : '', $spec_code);
        ...
        if ($v_member_scope != 2)//neu don vi lua chon la cap so hoac huyen
        {
            ...
            {
                $v_unit_code = $district['C_UNIT_CODE'];
                $v_spec_code = $district['C_SPEC_CODE'];
                array_push($arr_return, $district);
                foreach ($arr_village as $key => $village)
                {
                    $v_parent_unit_code = $village['C_UNIT_CODE'];
                    $v_village_spec_code = $village['C_SPEC_CODE'];
                    if ($v_parent_unit_code != $v_unit_code OR $v_spec_code != $v_village_spec_code)
                    {
                        continue;
                    }
                    $village['C_NAME'] = ' -- ' . $village['C_NAME'];
    ...
    /**
     * lay thong tin tong hop theo linh vuc cua cac don vi cap xa
     * @param type $month
     * @param type $year
     * @param type $unit_code
     * @param type $spec_code
     * @return type
     */
    public function qry_synthesis_by_spec_of_village($month = '', $year = '', $member_code = '', $spec_code = '')
    {
        $condition_member = '';
        $condition_spec = '';
        if ($member_code != '')
        ...
        $sql = "SELECT
                    ...
                    HS.FK_VILLAGE_ID,
                    HS.C_SPEC_CODE,
                    (SELECT C_NAME FROM t_cores_list WHERE C_CODE = HS.C_SPEC_CODE) AS C_SPEC_NAME,
                    IF(KT.C_COUNT_KY_TRUOC>=0,KT.C_COUNT_KY_TRUOC,0) AS C_COUNT_KY_TRUOC,
                    COALESCE(HS.C_COUNT_TONG_TIEP_NHAN,0) AS C_COUNT_TONG_TIEP_NHAN,
                    COALESCE(HS.C_COUNT_TONG_TIEP_NHAN_TRONG_THANG,0) AS C_COUNT_TONG_TIEP_NHAN_TRONG_THANG,
                    COALESCE(HS.C_COUNT_DANG_THU_LY,0) AS C_COUNT_DANG_THU_LY,
                    ...
                    LEFT JOIN (SELECT
                                ...
                                FROM (SELECT
                                      ...
                                      WHERE MONTH(C_HISTORY_DATE) = '$month'
                                          AND YEAR(C_HISTORY_DATE) = '$year'
                                          AND COALESCE(FK_VILLAGE_ID,0) > 0
                                      GROUP BY C_SPEC_CODE,C_UNIT_CODE,FK_VILLAGE_ID) a
                                  LEFT JOIN t_ps_record_history_stat hs
                                    ON a.C_UNIT_CODE = hs.C_UNIT_CODE
                                      AND a.C_MAX_HISTORY_DATE = hs.C_HISTORY_DATE
                                      AND a.FK_VILLAGE_ID = hs.FK_VILLAGE_ID
                                  WHERE (1>0) $condition_spec
                                GROUP BY hs.C_SPEC_CODE,hs.C_UNIT_CODE,hs.FK_VILLAGE_ID) HS
                      ON M.C_PARENT_CODE = HS.C_UNIT_CODE
                        AND M.FK_VILLAGE_ID = HS.FK_VILLAGE_ID
                        LEFT JOIN (SELECT
                                        hs.C_UNIT_CODE,
                                        hs.FK_VILLAGE_ID,
                                        hs.C_SPEC_CODE,
                                        C_COUNT_DANG_THU_LY AS C_COUNT_KY_TRUOC
                                      FROM (SELECT
                                              C_UNIT_CODE,
                                              MAX(C_HISTORY_DATE)  AS C_MAX_HISTORY_DATE,
                                              FK_VILLAGE_ID,
                                              C_SPEC_CODE
                                            FROM t_ps_record_history_stat
                                            WHERE MONTH(C_HISTORY_DATE) = '8'
                                                AND YEAR(C_HISTORY_DATE) = '2014'
                                                AND COALESCE(FK_VILLAGE_ID,0) > 0
                                            GROUP BY C_SPEC_CODE,C_UNIT_CODE,FK_VILLAGE_ID) a
                                        LEFT JOIN t_ps_record_history_stat hs
                                          ON a.C_UNIT_CODE = hs.C_UNIT_CODE
                                            ...
                                            AND a.FK_VILLAGE_ID = hs.FK_VILLAGE_ID
                                      WHERE (1 > 0) $condition_spec
                                      GROUP BY hs.C_SPEC_CODE,hs.C_UNIT_CODE,hs.FK_VILLAGE_ID) KT
                             ON M.C_PARENT_CODE = KT.C_UNIT_CODE
                               AND M.FK_VILLAGE_ID = KT.FK_VILLAGE_ID
                  WHERE (1 > 0) $condition_member
                  ORDER BY HS.C_SPEC_CODE,HS.C_UNIT_CODE,HS.FK_VILLAGE_ID";
        return $this->db->getAll($sql);
    }

    /**
     * lay du lieu tong hop theo don vi cap huyen
     * @param type $month
     * @param type $year
     * @param type $unit_code
     * @param type $spec_code
     * @param type $order_unit
     * @return type
     */
    public function qry_synthesis_by_spec_of_district($month = '', $year = '', $unit_code = '', $spec_code = '', $order_unit = 0)
    {
        $condition = '';
        ...
        if ($unit_code != '')
        {
            $condition .= " AND HS.C_UNIT_CODE = '$unit_code'";
        }

        if ($spec_code != '')
        {
            $condition .= " AND HS.C_SPEC_CODE = '$spec_code'";
        }
        $prev_month = Date('m', mktime(0, 0, 0, $month - 1, date("d"), date("Y")));
        $sql = "SELECT 
                    M.C_CODE AS C_UNIT_CODE,
                    ...
                    C_COUNT_THUE,
                    C_COUNT_LUY_KE,
                    (SELECT COUNT(*) FROM t_ps_member WHERE FK_MEMBER = M.PK_MEMBER) AS C_COUNT_VILLAGE
                FROM (SELECT
                        C_UNIT_CODE,
                        MAX(C_HISTORY_DATE) AS C_MAX_HISTORY_DATE,
                        FK_VILLAGE_ID
                  ...
                  ON HS.C_UNIT_CODE = M.C_CODE
                  LEFT JOIN (SELECT
                                hs.C_UNIT_CODE,
                                hs.C_SPEC_CODE,
                                hs.C_COUNT_DANG_THU_LY AS C_COUNT_KY_TRUOC
                              FROM (SELECT
                                      C_UNIT_CODE,
                                      MAX(C_HISTORY_DATE)                  AS C_MAX_HISTORY_DATE,
                                      FK_VILLAGE_ID
                                ...
                                LEFT JOIN t_ps_record_history_stat hs
                                  ON a.C_UNIT_CODE = hs.C_UNIT_CODE
                                    AND a.C_MAX_HISTORY_DATE = hs.C_HISTORY_DATE
                                    AND a.FK_VILLAGE_ID = hs.FK_VILLAGE_ID) KT
                     ON KT.C_UNIT_CODE = M.C_CODE
                       AND KT.C_SPEC_CODE = HS.C_SPEC_CODE
                  Where M.C_STATUS = 1 AND (M.C_SCOPE = 0 OR M.C_SCOPE = 1) $condition
                GROUP BY HS.C_UNIT_CODE,C_SPEC_CODE ";
        if ($order_unit == 1)
        {
            $sql .= ' ORDER BY HS.C_UNIT_CODE,C_SPEC_CODE';
        }
        else
        {

            $sql .= ' ORDER BY C_SPEC_CODE';
        }
     ...
     * @return type
     */
    public function qry_synthesis_chart($month = '', $year = '', $member_id = '', $spec_code = '')
    {
        $arr_member_info = array();
        ...
        if ($member_id != '')//don vi thanh vien
        {
            //lay thong tin don vi thanh vien
            $sql = "SELECT
                        PK_MEMBER,
                        ...
                        (SELECT
                         ...
                         WHERE PK_MEMBER = M.FK_MEMBER) AS C_PARENT_CODE
                      FROM t_ps_member M
                      WHERE PK_MEMBER = '$member_id'";
            $arr_member_info = $this->db->GetRow($sql);

            $v_member_scope = empty($arr_member_info['C_SCOPE']) ? 0 : $arr_member_info['C_SCOPE'];
            $v_member_code = empty($arr_member_info['C_CODE']) ? '' : $arr_member_info['C_CODE'];
            $v_member_parent_code = empty($arr_member_info['C_PARENT_CODE']) ? '' : $arr_member_info['C_PARENT_CODE'];
            $v_village_id = empty($arr_member_info['FK_VILLAGE_ID']) ? '' : $arr_member_info['FK_VILLAGE_ID'];

            if ($v_member_scope == 2)
            {
                $condition = " AND COALESCE(FK_VILLAGE_ID,0) = '$v_village_id' AND C_UNIT_CODE = '$v_member_parent_code'";
            }
            else
            {
                $condition = " AND COALESCE(FK_VILLAGE_ID,0) = 0 AND C_UNIT_CODE = '$v_member_code'";
            }
        }
     ...
     * @return type
     */
//    public function qry_synthesis_by_month($month)
//    {
//        $stmt = "SELECT
//                    a.C_UNIT_CODE,
//                    (SELECT
//                       C_NAME
//                     FROM t_ps_member
//                     WHERE C_CODE = HS.C_UNIT_CODE) AS C_NAME,
//                    SUM(C_COUNT_TONG_TIEP_NHAN) AS C_COUNT_TONG_TIEP_NHAN,
//                    SUM(C_COUNT_TONG_TIEP_NHAN_TRONG_THANG) AS C_COUNT_TONG_TIEP_NHAN_TRONG_THANG,
//                    SUM(C_COUNT_DANG_THU_LY) AS C_COUNT_DANG_THU_LY,
//                    SUM(C_COUNT_DANG_CHO_TRA_KET_QUA) AS C_COUNT_DANG_CHO_TRA_KET_QUA,
...
//                  FROM (SELECT
//                          C_UNIT_CODE,
//                          MAX(C_HISTORY_DATE) AS C_HISTORY_DATE
//                        FROM t_ps_record_history_stat
//                        WHERE MONTH(C_HISTORY_DATE) = ?
//                        GROUP BY C_UNIT_CODE) a
//                    LEFT JOIN t_ps_record_history_stat HS
//                      ON a.C_UNIT_CODE = HS.C_UNIT_CODE
//                        AND a.C_HISTORY_DATE = HS.C_HISTORY_DATE
//                    RIGHT JOIN t_ps_member M
//                      ON M.C_CODE = HS.C_UNIT_CODE 
//                    WHERE M.C_STATUS = 1
//                  GROUP BY HS.C_UNIT_CODE";
//        return $this->db->getAll($stmt,array((int)$month));
//    }
    ...
    public function qry_all_record_type_guidance()
    {
        ...
        $stmt = "Select
                        ...
                        (Select
                         ...
                         From t_ps_record_type rt, (SELECT @rownum:=$v_start)AS r
                         Where rt.C_STATUS = 1
                               $conditions
                      Order By C_ORDER ASC 
                      limit $v_start,$limit";
    ...
    public function qry_all_evaluation($v_staff_id = 0, $v_page_singel_limit = 0, $v_district_id = 0, $v_village_id = 0, $v_txt_filter = '')
    {
        ...
        $limit = ((int) $limit > 0) ? ' LIMIT ' . (int) $limit : ' LIMIT ' . 10;
        ...
        if ($v_page_singel_limit > 0)
        {
            ...
            if ($v_curent_page > 1)
            {
                $v_start = ($v_page_singel_limit * ($v_curent_page - 1));
                $v_start = ($v_start > 0) ? $v_start : 0;
                $v_end = $v_start + $v_page_singel_limit - 1;
    ...
    public function do_register()
    {
        ...
        if (trim($v_username) == '' OR ! preg_match('/^([a-zA-Z0-9_]+){5,30}$/', $v_username))
        {
            $MODE_DATA['message'] = 'Tên tài khoản không hợp lệ vui lòng chọn tên tài khoản khác.';
            $MODE_DATA['code_error'] = 'txt_username';
            return $MODE_DATA;
        }
        if (trim($v_password) == '' OR trim($v_confirm_password) == '' OR ( trim($v_password) != trim($v_confirm_password)))
        {
            $MODE_DATA['message'] = 'Mật khẩu không hợp lệ hoặc mật khẩu nhập lại không đúng.';
            $MODE_DATA['code_error'] = 'txt_confirm_password';
            return $MODE_DATA;
        }
        if (trim($v_email) == '' OR ( trim($v_email) != trim($v_confirm_email)) OR ! filter_var($v_email, FILTER_VALIDATE_EMAIL))
        {
            $MODE_DATA['message'] = 'Email không hợp lệ hoặc email nhập lại không đúng.';
            $MODE_DATA['code_error'] = 'txt_confirm_email';
            return $MODE_DATA;
        }
        // check exists username
        if ($this->db->GetOne("Select COUNT(PK_CITIZEN) From t_ps_citizen Where C_USERNAME = ?", $v_username) > 0)
        {
            $MODE_DATA['message'] = 'Tên tài khoản này đã tồn tại. Vui lòng chọn tên tài khoản khác.';
            $MODE_DATA['code_error'] = 'txt_username';
            return $MODE_DATA;
        }
        ...
        {
            $MODE_DATA['message'] = 'Email này đã tồn tại. Vui lòng chọn Email khác.';
            $MODE_DATA['code_error'] = 'txt_email';
            return $MODE_DATA;
        }
        $v_challenge = get_post_var('recaptcha_challenge_field');
        $v_response = get_post_var('recaptcha_response_field');
        ...
        if (!$resp->is_valid)
        {
            $MODE_DATA['message'] = 'Mã xác thực không chính xác!';
            $MODE_DATA['code_error'] = 'txt_email';
            return $MODE_DATA;
        }
        ...
        if ($this->db->ErrorNo() == 0)
        {
            ...
            if ($v_citizen_id <= 0)
            {
                $MODE_DATA['message'] = 'Xảy ra lỗi trong quá trình cập nhật. Xin vui lòng thực hiện lại.';
                $MODE_DATA['code_error'] = NULL;
                return $MODE_DATA;
            }
            ...
            $MODE_DATA['PK_CITIZEN'] = $this->db->GetOne(" Select PK_CITIZEN From t_ps_citizen where C_USERNAME = '$v_username'");
    ...
    public function do_deleted_account_citizen($citizen_id = 0)
    {
        $citizen_id = intval($citizen_id);
        if ($citizen_id > 0)
        {
            $this->db->Execute('Delete from t_ps_citizen where PK_CITIZEN = ?', array($citizen_id));
            $this->db->Execute('Delete from t_ps_citizen_tmp where FK_CITIZEN = ?', array($citizen_id));
        }
    }
...
//         get array t_ps_citizen_tmp by username
    ...
    public function do_login($v_username = '', $v_password = '')
    {
        ...
        {
            $MODE_DATA['error'] = 'Tài khoản hoặc mật khẩu không chính xác.';
            return $MODE_DATA;
        }
    ...
    public function qry_all_record($arr_filter = array())
    {
        ...
        return $this->db->GetAll("
                        SELECT 	
                            ...
                            PK_RECORD, 
                             Case When (R.C_REJECTED <> 0) 
                             Then 3 When ((R.C_REJECTED = 0 OR R.C_REJECTED IS NULL) And (R.C_CLEAR_DATE Is Not Null)) Then 2 Else 1 End as C_ACTIVITY,
    ...
    public function qry_all_help()
    {
        $sql = "SELECT *
                ...
                WHERE FK_LISTTYPE = (SELECT
                                       PK_LISTTYPE
                                     FROM t_cores_listtype
                                     WHERE C_CODE = '" . _CONST_DM_HELP . "'
                                         AND C_STATUS = 1)
                    AND C_STATUS = 1";
        return $this->db->GetAll($sql);
    }

    public function qry_single_help($v_id)
    {
        $sql = "SELECT *
                FROM t_cores_list
                WHERE FK_LISTTYPE = (SELECT
    ...
    public function qry_all_public_feedback($website_id)
    {
        $sql = "Select
                  ...
                  Where FK_WEBSITE = $website_id
                      And C_PUBLIC = 1
                  Order by C_INIT_DATE";

        return $this->db->getAll($sql);
    }

    function qry_all_faq()
    {
        return $this->db->GetAll("SELECT * FROM t_ps_faq WHERE fk_website=? AND c_verified=1 ORDER BY c_order", array($this->website_id));
    }

    /**
     * Gui thong tin lien he
     */
    function send_contact_email()
    {
        ...
        require $captcha_url . 'securimage.php';
        $securimage = new Securimage();
        if ($securimage->check(get_post_var('captcha_code')) == false)
        {
            return 'Bạn cần nhập lại mã xác nhận';
        }

        $arr_info = array(
            'ten'       => 'Tên đơn vị/cá nhân',
            'daiDien'   => 'Người đại diện',
            'email'     => 'Email',
        ...
        {
            $user_input = get_post_var($k);
            if ($user_input)
            {
                $html .= "<strong>$v</strong>:<br>" . $user_input . "<br><br>";
            }
        }
        $email_adress = get_system_config_value(CFGKEY_MAIL_ADD);
        $email_stmt = get_system_config_value(CFGKEY_MAIL_SERVER);
        $email_account = get_system_config_value(CFGKEY_MAIL_ACCOUNT);
        $email_pass = get_system_config_value(CFGKEY_MAIL_PASSWORD);
        $unit_info = get_system_config_value(CFGKEY_UNIT_NAME);
        ...
        $message = Swift_Message::newInstance()
                ...
                ->setSubject("[Website $unit_info] Thông tin liên hệ Website ")

                // Set the From address with an associative array
                ->setFrom(array($email_adress => 'Hệ thống'))

                // Set the To addresses with an associative array
                ->setTo(array($email_adress))

                // And optionally an alternative body
                ->addPart($html, 'text/html');

        // Create the Transport
        $transport = Swift_SmtpTransport::newInstance($email_stmt, 25, true)
    ...
    public function arr_all_song_ma_online($website_id)
    {
        $category_id = $this->db->GetOne("SELECT PK_CATEGORY FROM t_ps_category WHERE C_SLUG = 'song-ma-online'");
        return $this->qry_single_category($website_id, $category_id);
    }
    
    public function qry_all_tin_trong_huyen($website_id,$slug ='tin-trong-huyen')
    {
        $category_id = $this->db->GetOne("SELECT PK_CATEGORY FROM t_ps_category WHERE C_SLUG = '$slug'");
        return $this->qry_single_category($website_id, $category_id);
    }
