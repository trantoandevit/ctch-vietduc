...
//        $arr_single_website['C_THEME_CODE'] = 'mobile-2014';
        ...
        // anh thubnail default khi hien thi tin bai khong co thubnail
        $this->view->image_default_sticky = CONST_SITE_THEME_ROOT . DS . 'images' . DS . _CONST_IMAGE_DEFAULT_STICKY;
...
//        $this->_check_login_cookie();   
        ...
        //theme config 
        $theme_info['unit_address'] = get_system_config_value('unit_address');
        $theme_info['unit_phone'] = get_system_config_value('unit_phone');
        $theme_info['unit_hotline'] = get_system_config_value('unit_hotline');
        $theme_info['unit_name'] = get_system_config_value('unit_name');
        $theme_info['unit_fax'] = get_system_config_value('unit_fax');
        $theme_info['unit_email'] = get_system_config_value('unit_email');
        
        $theme_info['medical_register_hotline'] = get_system_config_value('medical_register_hotline');
        
        
        $this->view->theme_config = $theme_info;
    }
    ...
    private function get_default_data()
    {
        ...
        {
            $VIEW_DATA['arr_all_breaking_news'] = $this->model->gp_qry_all_breaking_news($this->website_id);
        }
        return $VIEW_DATA;
    }
...
//
//        //Lay du lieu TIN NOI BAT cua trang chu (sticky)
//        if (!set_viewdata_data($this->website_id, 'sticky', $VIEW_DATA['arr_all_sticky']))
//        {
//            $VIEW_DATA['arr_all_sticky'] = $this->model->gp_qry_all_sticky($this->website_id);
//        }
...
//        {
//            $VIEW_DATA['arr_all_cat_art'] = $this->model->gp_qry_all_featured_category($this->website_id);
//        }
//        
//        //Danh sach tin bai moi nhat
//        if (!set_viewdata_data($this->website_id, 'latest_article', $VIEW_DATA['arr_all_latest_article']))
//        {
//            $VIEW_DATA['arr_all_latest_article'] = $this->model->gp_qry_all_latest_article($this->website_id);
//        }
//        
//        //Lay du lieu su kien
//        if (!set_viewdata_data($this->website_id, 'event', $VIEW_DATA['arr_all_event']))
//        {
//            $VIEW_DATA['arr_all_event'] = $this->model->qry_all_event($this->website_id);
//        }
//        $VIEW_DATA['arrNewDoc'] =  $this->model->qry_all_new_doc(20);
//        
//        
//        /**
//         * Load danh tin bài nổi bật của chuyên mục co slugs:song-ma-online 
//         */
//        //Lay du lieu su kien
//        if (!set_viewdata_data($this->website_id, 'ong_ma_online', $VIEW_DATA['arr_all_event']))
//        {
//            $VIEW_DATA['arr_all_song_ma_online'] = $this->model->arr_all_song_ma_online($this->website_id);
//        }
//        
//        /**
//         * Tin bai trong huyen
//         */
//        if (!set_viewdata_data($this->website_id, 'tin_trong_huyen', $VIEW_DATA['arr_all_event']))
//        {
//            $VIEW_DATA['arr_all_tin_trong_huyen'] = $this->model->qry_all_tin_trong_huyen($this->website_id,'tin-trong-huyen');
//        }
    ...
    public function dsp_category()
    {
        $VIEW_DATA = array();
        $this->view->render('dsp_category', $VIEW_DATA, $this->theme_code);
    }
...
//        $VIEW_DATA = $this->get_default_data();
    ...
    public function dsp_lookup()
    {
        ...
        $this->view->render('internet_record/dsp_lookup', $VIEW_DATA, $this->theme_code);
    }
...
//        $month = '06';
        ...
        $VIEW_DATA['arr_synthesis_chart'] = $this->model->qry_synthesis_by_spec_of_district($month, $year, '', '', 1);

        $VIEW_DATA['arr_table_synthesis'] = $this->model->qry_synthesis_of_district($month, $year, '', '', 1);

        $VIEW_DATA['arr_synthesis_of_village'] = $this->model->qry_synthesis_of_village($month, $year);
    ...
    public function dsp_guidance_internet_record()
    {
        ...
        $this->view->render('guidance/dsp_guidance_internet_record', $VIEW_DATA, $this->theme_code);
    }
    ...
    public function dsp_list_internet_record()
    {
        ...
        $v_fields_id = get_request_var('sel_fields_id', '');
        $v_member_id = get_request_var('sel_member', 0);
        $v_text_filter = get_request_var('text_filter', 0);

        $this->view->active_menu_top = 'public_service';

        //Danh sach linh vuc tiep nhan ho so truc tuyen
        $VIEW_DATA['arr_all_fileds_level_3'] = $this->model->qry_all_fileds_level_3();

        //Danh sach don vi tiep nhan ho so truc tuyen
        $VIEW_DATA['arr_all_member'] = $this->model->qry_all_member_receive_internet();

        //Danh sach thu tuc nop ho so truc tuyen
        $VIEW_DATA['arr_all_record_type'] = $this->model->qry_all_record_receive_internet($v_fields_id, $v_member_id, $v_text_filter);

        //render
        $this->view->render('internet_record/dsp_list_internet_record', $VIEW_DATA, $this->theme_code);
    }

    /**
     * Xem chi tiet huong dan thu tuc chon nop ho so truc tuyen
     */
    public function dsp_single_record_type()
    {
        ...
        //Chi tiet thu tuc nop ho so truc tuyen
        $VIEW_DATA['arr_single_record_type'] = $this->model->qry_single_record_type($v_record_type_id);

        //render
        $this->view->render('internet_record/dsp_single_record_type', $VIEW_DATA, $this->theme_code);
    }
    ...
    public function dsp_submit_internet_record()
    {
        ...
        $VIEW_DATA['arr_all_member'] = $this->model->qry_all_member_receive_internet($record_type_id);
        ...
        $this->view->render('internet_record/dsp_submit_internet_record', $VIEW_DATA, $this->theme_code);
    }
    ...
    private function dsp_synthesis_chart($v_method)
    {
        switch ($v_method)
        {
            ...
            case 'so_sanh':
                ...
                if ($v_compare_type == 0)
                {
                    $VIEW_DATA['arr_synthesis_chart'] = $this->model->qry_synthesis_of_district($v_month, $v_year);
                }
                else
                {
                    $VIEW_DATA['arr_synthesis_chart'] = $this->model->qry_synthesis_of_village($v_month, $v_year);
                }
    ...
    private function dsp_synthesis_member($v_method)
    {
        ...
        switch ($v_method)
        {
            ...
            case 'tiep_nhan':
                ...
                break;
            default:
                $this->view->render('dsp_dv_tong_hop', $VIEW_DATA);
                break;
        }
    }
    ...
    private function dsp_synthesis_spec($v_method)
    {
        ...
        switch ($v_method)
        {
            ...
            case 'tiep_nhan':
            default:
                $this->view->render('dsp_lv_tong_hop', $VIEW_DATA);
                break;
        }
    }
...
//widget weblink
    ...
    private function dsp_widget_list_menu_category($args = '')
    {
        try
        {
            $args = @new SimpleXMLElement($args, LIBXML_NOCDATA);
            $title = (string) $args->title;
            $menu_display = (string) $args->menu_display;
            $title_list_menu_category = (string) $args->title_list_menu_category;
        }
        catch (Exception $ex)
        {
        ...
        $data['arr_all_menu'] = $this->model->gp_qry_all_menu_position($this->website_id, $menu_display);
//        $data['menu_html'] = $html;
        $data['title_list_menu_category'] = $title_list_menu_category;
        $data['title'] = $title;

        ob_start();
        $this->view->render('dsp_widget_list_menu_category', $data);
        return ob_get_clean();
    }
    ...
    private function dsp_widget_video_clip($args = '')
    {
        ...
        try
        {
            ...
            $data['title'] = (string) $args->txt_video_clip_title;
        }
        catch (Exception $ex)
        {
...
//end
//Lay thong tin tinh trang xu ly ho so mot cua
    private function dsp_widget_tinh_trang_xu_ly_ho_so($args = '')
    {
        $data['widget_title'] = '';
        $data['txt_title'] = '';
        $data['txt_url'] = '';
        ...
        {
            $args = @new SimpleXMLElement($args, LIBXML_NOCDATA);
            $data['widget_title'] = (string) $args->txt_title;
            $data['txt_title'] = (string) $args->txt_title;
            $data['txt_url'] = (string) $args->txt_url;
            $data['txt_account'] = (string) $args->txt_account;
            $data['txt_password'] = (string) $args->txt_password;
            $data['txt_uri'] = (string) $args->txt_uri;
        }
        catch (Exception $ex)
        {
        ...
        ob_start();
        $this->view->render('dsp_widget_tinh_trang_xu_ly_ho_so', $data);
        $html = ob_get_clean();
        return $html;
    }
...
//        }
    ...
    public function dsp_guidance()
    {
        ...
        $this->view->render('guidance/dsp_guidance', $VIEW_DATA, $this->theme_code);
    }
    ...
    public function dsp_single_guidance()
    {
        ...
        $this->view->render('guidance/dsp_single_guidance', $VIEW_DATA, $this->theme_code);
    }
...
//        $VIEW_DATA['arr_all_widget_position'] = $this->prepare_all_widget_position(PAGE_ARCHIVE);
    ...
    public function dsp_evaluation_results()
    {
        ...
        if ($v_staff_id <= 0)
        {
            ...
            $this->view->render('guidance/dsp_evaluation_results', $VIEW_DATA, $this->theme_code);
        }
        else
        {
            $arr_single_staff = $this->model->qry_staff_get_by_id($v_staff_id);
            ...
            $this->view->render('guidance/dsp_single_evaluatosion_resuslt', $VIEW_DATA, $this->theme_code);
        }
    }
    ...
    public function register()
    {
        ...
        $this->view->render('account_citizen/dsp_register', $VIEW_DATA, $this->theme_code);
    }
    ...
    public function do_register()
    {
        try
        {
            if ($this->check_citizen_login())
            {
                throw new Exception('Bạn không thể tạo tài khoản khi đang đăng nhập!!!');
            }
            $VIEW_DATA['code_error'] = '';
            $arr_result = $this->model->do_register();
            if (isset($arr_result['message']))
            {
                $VIEW_DATA['code_error'] = isset($arr_result['code_error']) ? $arr_result['code_error'] : NULL;
                throw new Exception($arr_result['message']);
            }
            else
            {
                ...
                ob_start();
                $this->view->render('account_citizen/dsp_content_send_mail', $VIEW_DATA, $this->theme_code);
                $html = ob_get_clean();
                //send mail
                ...
                if (!$reslut_sen_mail)
                {
                    //Đăng ký thất bại xóa tất cả thông tin đã lưu
                    $this->model->do_deleted_account_citizen($VIEW_DATA['PK_CITIZEN']);
                    $VIEW_DATA['message'] = 'Xảy ra lỗi trong quá trình thực hiện !!!' . $reslut_sen_mail;
        ...
        {
            $VIEW_DATA['message'] = $ex->getMessage();
            $VIEW_DATA['status'] = 'error';
        }

        if (!isset($VIEW_DATA['status']))
        {

            $VIEW_DATA['message'] = 'Bạn đã đăng ký thành công!!!';
...
//    Hien thi giao dien kich hoat
    public function dsp_do_account_trigger()
    {
        ...
        if (trim($v_username) == '')
        {
            echo '<h2 style="width:100%;text-align:center; color:Red;">Tài khoản này không tồn tại hoặc đã được kích hoạt.<br/> <a class="btn btn-primary" style="color:white" href="' . SITE_ROOT . '">Trang chủ</a></h2>';
            exit();
        }
        ...
        $this->view->render('account_citizen/dsp_do_account_trigger', $VIEW_DATA, $this->theme_code);
    }
    ...
    public function do_login()
    {
        ...
        {
            return __('Vui lòng nhập đầy đủ tên tài khoản và mật khẩu.');
        }
    ...
    public function do_logout()
    {
        ...
        Session::session_unset('citizen_name');
        Session::session_unset('citizen_email');
        Session::session_unset('account_' . Session::get('citizen_login_name'));
        Session::session_unset('citizen_role');
    ...
    public function dsp_single_account()
    {
        ...
        $this->view->render('account_citizen/dsp_single_citizen_account', $VIEW_DATA, $this->theme_code);
    }
...
//        header("Refresh: 0; url=$page");
    ...
    public function dsp_history_filing()
    {
        ...
        if (trim($v_detail_record_no) != '')
        {
            ...
            if (sizeof($arr_single_record) == 1)
            {
                ...
                if ($v_status == -1)
                {
                    ...
                    $v_html_detail_form = "<h3 style='margin:8px;'>" . __('Hồ sơ có mã "' . $v_detail_record_no . '" bị từ chối. Do nội dung không đúng hoặc không hợp lệ. Mọi thắc mắc xin vui lòng liên hệ với nhà cung cấp dịch vụ.') . " </h3><hr/>";
                    ;
                }
                else if ($v_status == 1)
                {
                    ...
                    //Lay ma thu tuc
                    $v_record_type_code = preg_replace('/([A-Z-0-9]+)-([A-Z-0-9]+)-([A-Z-0-9]+)/', '$1', $v_detail_record_no);

                    //lay url
                    $v_url = $arr_loockup_link[$v_short_code]['C_LOOKUP_LINK'];
                    $v_url_record_type = $arr_loockup_link[$v_short_code]['C_LOOKUP_TYPE_LINK'];
                    //lay html

                    $v_html_detail_record = file_get_contents($v_url . $v_detail_record_no);
                    $v_xml_form_truct = file_get_contents($v_url_record_type . $v_record_type_code);

                    if (trim($v_xml_data) != '')
                    {
                        @$dom = simplexml_load_string($v_xml_form_truct);
                        if ($dom)
                        {
                            $v_html_detail_form = $this->view->transform(FALSE, $v_xml_form_truct);
                        }
                        else
                        {
                            $v_html_detail_form = '<h3  style="margin:8px;">Đã xảy lỗi. Vui lòng thử lại sau.</h3>';
                        }
                    }
                    else
                    {
                        ...
                        {
                            //TH1: Ho so chua duoc duyet hoac da bi xoa tren cong
                            $v_html_detail_record = "<h3 style='margin:8px;'>" . __('Hồ sơ có mã "' . $v_detail_record_no . '" đã được duyệt nhưng đơn vị tiếp nhận chưa xử lý. Mọi thắc mắc vui lòng liên hệ với nhà cung cấp dịch vụ.') . " </h3><hr/>";
                        }
        ...
        $this->view->render('account_citizen/dsp_history_filing', $VIEW_DATA, $this->theme_code);
    }
    ...
    public function dsp_active_change_email()
    {
        ...
        $this->view->render('account_citizen/dsp_active_change_email', $VIEW_DATA, $this->theme_code);
    }
    ...
    {
        $arr_result = $this->model->send_email_activation_code();
        if (sizeof($arr_result) == 0)
        {
            echo 0;
            return false;
        }
        $VIEW_DATA['username'] = $arr_result['username'];
        $VIEW_DATA['email'] = $arr_result['email'];
        $VIEW_DATA['code'] = $arr_result['code'];
        $VIEW_DATA['create_date'] = $arr_result['create_date'];
        $VIEW_DATA['citizen_id'] = $arr_result['citizen_id'];
        $VIEW_DATA['type'] = 'trigger_email';
        ob_start();
        ...
        require SERVER_ROOT . 'libs' . DS . 'mail_sender.php';
        $mail_sender = new MailSender();
        @$reslut_sen_mail = $mail_sender->SendMail($VIEW_DATA['email'], $html, 'text/html');
        echo 1;
        return true;
    }
            ...
            if ($id_user > 0)
            {
                ...
                if ($citizen_tmp_code)
                {
                    ...
                    if ($reslut_sen_mail > 0)
                    {
                        $VIEW_DATA = array();
                        $VIEW_DATA['email'] = $email;
                        $this->view->render('account_citizen/dsp_send_request_success', $VIEW_DATA, $this->theme_code);
                    }
                    else
                    {
    ...
    public function dsp_form_request_password()
    {
        ...
        $this->view->render('account_citizen/dsp_form_request_password', $VIEW_DATA, $this->theme_code);
    }
    ...
    public function dsp_change_password()
    {
        ...
        if (get_request_var('email') && get_request_var('v_code'))
        {
            ...
            {
                $VIEW_DATA['email'] = $email;
                $this->view->render('account_citizen/dsp_change_password', $VIEW_DATA, $this->theme_code);
            }
            else
            {
                $url_page = SITE_ROOT;
    ...
    public function dsp_help()
    {
        $VIEW_DATA = $this->get_default_data();
        $v_id = get_request_var('id', 0);
        //Lay danh sach noi dung huong dan
        $this->view->active_menu_top = 'help';
        $VIEW_DATA['arr_all_help'] = $this->model->qry_all_help();
        $VIEW_DATA['arr_singe_help'] = isset($VIEW_DATA['arr_all_help'][0]) ? $VIEW_DATA['arr_all_help'][0] : array();
        if ($v_id > 0)
        {
            $VIEW_DATA['arr_singe_help'] = $this->model->qry_single_help($v_id);
        }
        $this->view->render('dsp_help', $VIEW_DATA, $this->theme_code);
    }
    ...
    function faq()
    {
        ...
        //lay danh sach feedback public
        $VIEW_DATA['arr_all_public_feedback'] = $this->model->qry_all_public_feedback($website_id);

        $VIEW_DATA['arr_all_faq'] = $this->model->qry_all_faq();

        $this->view->render('faq', $VIEW_DATA, $this->theme_code);
    }

    function dsp_contact()
    {
        if ($_POST)
        {
            $err = $this->model->send_contact_email();
            if ($err)
            {
                echo $err;
            }
        ...
        $website_id = $this->website_id;
        $event_id = get_request_var('event_id', 0);
        $this->view->active_menu_top = 'contact';
        ...
        $this->view->render('dsp_contact', $VIEW_DATA, $this->theme_code);
    }

    function contact_success($success)
    {
        $website_id = $this->website_id;
        $event_id = get_request_var('event_id', 0);
        $this->view->menu_active = 'concatenation';

        $VIEW_DATA = $this->_get_default_data();
        //Lay du lieu TIN NOI BAT trong ngay
        $VIEW_DATA['arr_all_widget_position'] = $this->prepare_all_widget_position(PAGE_SINGLE);
        //lay danh sach feedback public
        $VIEW_DATA['arr_all_public_feedback'] = $this->model->qry_all_public_feedback($website_id);
        ...
        $this->view->render('dsp_contact_success', $VIEW_DATA, $this->theme_code);
    }

    /**
     * 8-7-2015
     * @author PhamHuong 
     * Hien thi chuyen muc video
     */
    function dsp_list_video()
    {
        $website_id = $this->website_id;
        $event_id = get_request_var('event_id', 0);
        $this->view->menu_active = 'photo_gallery_video';

        $VIEW_DATA = $this->get_default_data();
        $VIEW_DATA['arr_all_widget_position'] = $this->prepare_all_widget_position(PAGE_ARCHIVE);

        $VIEW_DATA['arr_all_video'] = $this->model->qry_new_video(_CONST_DEFAULT_ROWS_PER_PAGE);
        $this->view->render('dsp_list_video', $VIEW_DATA, $this->theme_code);
    }

    public function dsp_single_gallery()
    {
        $website_id = $this->website_id;
        $gallery_id = get_request_var('gallery_id', 0);

        $VIEW_DATA = $this->get_default_data();
        $VIEW_DATA['arr_all_widget_position'] = $this->prepare_all_widget_position(PAGE_ARCHIVE);

        $VIEW_DATA['arr_single_gallery'] = $this->model->qry_single_gallery($website_id, $gallery_id);
        $VIEW_DATA['arr_all_widget_position'] = $this->prepare_all_widget_position(PAGE_SINGLE);
        $this->view->render('dsp_single_gallery', $VIEW_DATA, $this->theme_code);
    }

    /**
     * danh sach tat ca phong su anh
     */
    public function dsp_all_gallery()
    {
        $website_id = $this->website_id;
        $event_id = get_request_var('event_id', 0);
        ...
        $VIEW_DATA['arr_all_photo_gallery'] = $this->model->qry_all_photo_gallery($website_id, $event_id);

        $this->view->render('dsp_all_gallery', $VIEW_DATA, $this->theme_code);
    }

    /**
     * Đồng bộ dữ liệu từ phần mềm một cửa
     */
    public function load_prgess_record_r3()
    {
//        header('Content-type: text/json');
        $txt_url = get_request_var('txt_url', '');
        $txt_account = get_request_var('txt_account', '');
        $txt_password = get_request_var('txt_password', '');

        $client = new SoapClient($txt_url);
        $xml_header = "<UserId>$txt_account</UserId><PassWord>$txt_password</PassWord>";
        $set_token = new SoapVar($xml_header, XSD_ANYXML);
        $SoapHeader = new SoapHeader('urn:http://tamviettech.vn', 'Header', $set_token);
        try
        {
            //call services get progess Don vi cap huyen
            $progress_destrict = $client->__soapCall('progress_report', array('unit_id' => '0'
                , 'month'   => date('m')
                , 'year'    => date('Y'))
                    , array()
                    , array($SoapHeader)
            ...
            //call services get progess don vi cap xa
            $progress_village = $client->__soapCall('progress_report', array('unit_id' => '-1'
                , 'month'   => date('m')
                , 'year'    => date('Y'))
                    , array()
                    , array($SoapHeader)
        ...
        if (is_soap_fault($progress_destrict) OR is_soap_fault($progress_village))
        {
            echo json_encode(array('response' => 'fault', 'msg' => 'Xảy ra sự cố khi đồng bộ dữ liệu. [Code:' . __LINE__ . ']'));
            exit();
        }


        $dom = new DOMDocument;
        $dom->loadXML($progress_destrict); // $xml is the XML string
        $xpath = new DOMXPath($dom);
        $district_sum_som_han = (int) $xpath->evaluate('sum(//row/C_COUNT_TRA_SOM_HAN)');
        $district_sum_dung_han = (int) $xpath->evaluate('sum(//row/C_COUNT_TRA_DUNG_HAN)');
        $district_sum_qua_han = (int) $xpath->evaluate('sum(//row/C_COUNT_TRA_QUA_HAN)');

        $dom->loadXML($progress_village); // $xml is the XML string
        $xpath = new DOMXPath($dom);
        $village_sum_som_han = (int) $xpath->evaluate('sum(//row/C_COUNT_TRA_SOM_HAN)');
        $village_sum_dung_han = (int) $xpath->evaluate('sum(//row/C_COUNT_TRA_DUNG_HAN)');
        $village_sum_qua_han = (int) $xpath->evaluate('sum(//row/C_COUNT_TRA_QUA_HAN)');


        $v_sum_da_tra = $district_sum_som_han + $district_sum_dung_han + $district_sum_qua_han + $village_sum_som_han + $village_sum_dung_han + $village_sum_qua_han;
        $v_sum_da_tra_som_va_dung = $district_sum_som_han + $district_sum_dung_han + $village_sum_som_han + $village_sum_dung_han;
        $tyle = '100';
        if ($v_sum_da_tra > 0)
        {
            $tyle = round($v_sum_da_tra_som_va_dung / $v_sum_da_tra, 4) * 100;
        }
        $tyle = $tyle . '%';
        $return = json_encode(array('response' => 'done', 'msg' => 'Thành công. [Code:' . __LINE__ . ']', 'tyle' => $tyle));
        echo $return;
        exit();
    }
    ...
    public function dsp_all_van_ban_phap_quy()
    {
        ...
        // get arrData
        $condition = "";
        $txt_title = get_get_var('txt_title','');
        $txt_cqbh = get_get_var('sel_cqbh','');
        $txt_lvtk = get_get_var('sel_lvtk','');
        $txt_loai_vb = get_get_var('sel_loai_vb','');
        $page = get_get_var('page',1);
